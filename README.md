...
immediately after that trailing space. A bare `--` immediately before the cursor (e.g.
`left--right` -> `left— right`) expands to an em dash followed by a trailing space (`— `,
U+2014 U+0020) with the cursor placed right after that trailing space; `---` and longer
hyphen runs are left untouched so Markdown thematic breaks, YAML frontmatter delimiters,
and other multi-hyphen text keep Obsidian's normal Tab behavior.
...
planner coverage, Pomodoro entry move-and-rename planner coverage,
