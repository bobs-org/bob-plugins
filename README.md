...
| Bob Ledger Tools        | `bob-ledger-tools`       |   1.3.0 | Expand Bob daily-note snippets and ledger time ranges, and navigate and adjust Pomodoro entries. |
...
`left--right` -> `left—right`) expands to a single em dash (`—`, U+2014) with the cursor
placed right after it; `---` and longer hyphen runs are left untouched so Markdown
thematic breaks, YAML frontmatter delimiters, and other multi-hyphen text keep Obsidian's
normal Tab behavior.
