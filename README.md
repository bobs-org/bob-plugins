...
| ----------------------- | ------------------------ | ------: | --------------------------------------------------------------------- |
| Block ID Prompt         | `block-id-prompt`        |   1.7.0 | Prompt for custom block IDs, complete wiki block links to open tasks (skipping `#hide` tasks and listing Blocked `[?]` tasks in a separate group beneath unblocked tasks), prune duplicate links from future open Pomodoros, mark dependency-blocked tasks, and — when a `^^` task-picker link is the sole content of an open Pomodoro sub-bullet — pull a uniquely future-scheduled target into today by removing its `scheduled` date, promoting Blocked/Ready to Next, and prepending a dated entry to an existing Schedule Log (never creating one for a task that keeps none); `Ctrl+Shift+Enter` links the open task under the cursor (including a `#hide` task) to today's current/next Pomodoro directly, prompting for a block ID first if it has none, always forcing the task to Next regardless of its prior status, and applying the same future-schedule/Schedule Log pull-forward as the `^^` flow. |
