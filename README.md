...
| Bob Navigation Hotkeys  | `bob-navigation-hotkeys` |   1.7.0 | Open parent/alternate notes and set bullet properties, including local task dependencies with visible `**DEPENDS ON:**` `[[#^block-id]]` navigation links and `<Tab>` batch add/remove — including batch selection of tasks that still need a block ID, collected via sequential block-ID prompts before the batch is applied. |
